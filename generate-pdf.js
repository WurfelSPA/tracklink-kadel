...
'use strict';
/**
 * generate-pdf.js — KADEL
 * Clon parametrizado de generate-pdf.js (tracklink-santamarta) para el
 * "Informe de excesos de velocidad sobre 100 km/h" de KADEL.
 *
 * Diferencias vs. el original de Santa Marta:
 *   - CONFIG.siteName / CONFIG.footerLabel → "KADEL"
 *   - Paleta de color: azul marino / slate (vs. índigo-celeste de Santa Marta)
 *   - CONFIG.speedThreshold = 100 → filtra filas con velocidad < umbral
...
const CONFIG = {
  ...
  colorRankFrom:  '#1e2f52', // barras — extremo oscuro del degradé por ranking
  colorRankTo:    '#94a3b8', // barras — extremo claro (slate) del degradé por ranking
  colorAccent:    '#1a2744', // línea de pico / badge en gráficos verticales
};
...
// KADEL no tiene columna "Conductor" — no aplica corrección de nombres propios.
...
// Paleta KADEL: navy profundo → slate claro (vs. índigo-celeste de Santa Marta).
...
<div class="page"><div class="pi">
  ...
  <p class="pg-intro">Los cuatro indicadores clave del período permiten focalizar la intervención de la supervisión de flota sobre las unidades y horarios de mayor riesgo.</p>
  <div class="kpi-grid">
    <div class="kpi"><div class="kpi-val">${s.totalIncidencias}</div><div class="kpi-lbl">Total de Excesos</div><div class="kpi-desc">Incidencias sobre ${CONFIG.speedThreshold} km/h en el período completo</div></div>
    <div class="kpi"><div class="kpi-val">${fmtPct(s.conductoresArr[0]?.pct || 0)}%</div><div class="kpi-lbl">Unidad Crítica</div><div class="kpi-desc">${s.conductoresArr[0]?.name || '—'} concentra ${s.conductoresArr[0]?.count || 0} excesos del total</div></div>
    ...
    <div class="kpi"><div class="kpi-val">${pad(s.peakHour.hour)}h</div><div class="kpi-lbl">Horario Punta</div><div class="kpi-desc">Franja ${s.peakHour.fullLabel} concentra el máximo (${s.peakHour.count} excesos)</div></div>
  </div>
  <div class="alert alert-yellow"><span class="icon-warn"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 3L22 20H2L12 3Z" stroke="#eab308" stroke-width="2" stroke-linejoin="round" fill="#fef9c3"/><path d="M12 10v4M12 17h.01" stroke="#a16207" stroke-width="2" stroke-linecap="round"/></svg></span><div>El ${DIAS_ES_FULL[s.peakDay.date ? s.peakDay.date.getDay() : 0]} ${s.peakDay.label.split(' ')[1] || ''} fue el día más crítico del período, concentrando <strong>${s.peakDay.count} excesos (${fmtPct(s.totalIncidencias ? s.peakDay.count / s.totalIncidencias * 100 : 0)}% del total semanal)</strong>, atribuibles en gran parte a la unidad ${s.globalMaxConductor}.</div></div>
...
<div class="page"><div class="pi">
  ...
  <p class="pg-intro">La distribución es desigual: las tres primeras unidades acumulan el <strong>${fmtPct(s.top3Pct)}% del total</strong>, lo que indica la necesidad de intervención focalizada. ${top3Text ? `Lidera ${top3Names[0] || ''} (${s.conductoresArr[0]?.count || 0})${top3Names[1] ? `, ${top3Names[1]} (${s.conductoresArr[1]?.count || 0})` : ''}${top3Names[2] ? `, ${top3Names[2]} (${s.conductoresArr[2]?.count || 0})` : ''}.` : ''}</p>
  <div class="p3-row">
    ...
    <div class="p3-table-wrap">
      ...
      <p class="p3-note">${s.globalMaxConductor} registró la velocidad puntual más alta del período (${fmtSpeed(s.globalMaxSpeed)} km/h).</p>
    </div>
  </div>
  <div class="pf">${footer}</div>
</div></div>
...
<div class="page"><div class="pi">
  ...
  <p class="pg-intro">Las tres franjas horarias con más incidencias concentran el <strong>${fmtPct((top3Hours.reduce((s2,h)=>s2+h.count,0)/(s.totalIncidencias||1))*100)}%</strong> de los excesos. El <strong>${corePct}%</strong> ocurre en la franja ${coreLabel}. Las franjas más críticas son ${top3Hours.map(h=>`${h.hour}h`).join(', ')} (${s.peakHour.count} excesos c/u).</p>
  <div class="p4-chart-wrap">${hourChartSvg}</div>
  <div class="note-box"><span class="icon-info"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#64748b" stroke-width="2" fill="#e2e8f0"/><path d="M12 11v5M12 8h.01" stroke="#475569" stroke-width="2" stroke-linecap="round"/></svg></span><div>Las franjas de mayor riesgo (${coreLabel}) concentran la mayor parte de las incidencias. Se recomienda reforzar controles y señalización en estos horarios.</div></div>
  <div class="pf">${footer}</div>
</div></div>
  ...
  <div class="p5-row">
    ...
    <div class="p5-actions-col">
      <div class="concl-grid">
        <div class="concl-card"><h4>Intervención Focalizada</h4><p>Priorizar a las unidades ${top3Text} en programas de conducción segura y manejo defensivo; juntas concentran el ${fmtPct(s.top3Pct)}% de los excesos del período.</p></div>
        <div class="concl-card"><h4>Control en Horario Crítico</h4><p>Reforzar la supervisión en las franjas ${coreLabel}, donde se concentra la mayor cantidad de incidencias, y en el día más crítico (${criticalDayText}).</p></div>
        <div class="concl-card"><h4>Revisión de Unidad ${s.globalMaxConductor}</h4><p>Verificar el estado y el comportamiento de esta unidad, dado el registro de velocidad máxima de ${fmtSpeed(s.globalMaxSpeed)} km/h en el período.</p></div>
        <div class="concl-card"><h4>Seguimiento Continuo</h4><p>Establecer alertas automáticas para las unidades que superen los umbrales de excesos semanales definidos por la supervisión.</p></div>
      </div>
    </div>
  </div>
  <div class="pf">${footer}</div>
